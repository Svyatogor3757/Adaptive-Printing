...
dh = 0;
sub_offsetX = 0;
Rectangles = zeros(subimages_count, 2);
    ...
    if ~all(sub_find_white,'all')
       ...
       if length(sub_find_black_index) < 2
          if( sub_find_black_index > 1 && sum(sub(:,sub_find_black_index-1)) == 0)
              Rectangles(i,1) = sub_find_black_index;
              Rectangles(i,2)   = image_size(subimage_white_dim);
          else
              Rectangles(i,1) = 1;
              Rectangles(i,2)   = sub_find_black_index;
          end
       else
           Rectangles(i,1)= sub_find_black_index(1);
           Rectangles(i,2)  = sub_find_black_index(end);
       end
    end
    dh = dh2;
end
...
trackXY = [0 0];
sub_OffsetY = 0;
i = length(Rectangles(:,1));
direction = 0;
while i >= 1 
    if(Rectangles(i,1) == 0 || Rectangles(i,2) == 0)
        if i == 1
            %i = i;
            break;
        else
    ...
    [OFFSETX, j] = get_offsetX(Rectangles, i, subimages_margin, direction);
    if i == 2 && length(Rectangles(:,1)) ~= subimages_count
...
hold on;
pl=[];
for i=length(Rectangles(:,1)):-1:1
    if(Rectangles(i,1) == 0 || Rectangles(i,2) == 0)
        sub_OffsetY = sub_OffsetY + subimage_size;
        continue;
    end
    pl(end+1)= rectangle('Position', [Rectangles(i,1), sub_OffsetY, Rectangles(i,2)-Rectangles(i,1), subimage_size]);
    sub_OffsetY = sub_OffsetY + subimage_size;
    drawnow
end
...
for j=1:length(Rectangles(:,1))
    if(Rectangles(j,1) == 0 || Rectangles(j,2) == 0) 
        continue;
    end
    ...
    offsetX = 0;
    workspace((1+subimage_size*(j-1):subimage_size*j)+Ystart_workspace, (Rectangles(j,1)-line_width:Rectangles(j,1))+Xstart_workspace + offsetX, 1) = 200/255;    
    workspace((1+subimage_size*(j-1):subimage_size*j)+Ystart_workspace, (Rectangles(j,2):Rectangles(j,2)+line_width)+Xstart_workspace + offsetX, 1) = 200/255;
    for k=2:3
         workspace((1+subimage_size*(j-1):subimage_size*j)+Ystart_workspace, (Rectangles(j,1)-line_width:Rectangles(j,1))+Xstart_workspace + offsetX, k) = ~ workspace((1+subimage_size*(j-1):subimage_size*j)+Ystart_workspace, (Rectangles(j,1)-line_width:Rectangles(j,1))+Xstart_workspace + offsetX, k);    
         workspace((1+subimage_size*(j-1):subimage_size*j)+Ystart_workspace, (Rectangles(j,2):Rectangles(j,2)+line_width)+Xstart_workspace + offsetX, k) = ~ workspace((1+subimage_size*(j-1):subimage_size*j)+Ystart_workspace, (Rectangles(j,2):Rectangles(j,2)+line_width)+Xstart_workspace + offsetX, k);  
    end
end
trackXY(trackXY==0) = 1;
